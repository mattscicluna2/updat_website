...
/*Lord Icon ------------------------------------------------------*/
function saasland_lordicon_lists() {
    $icons = array();
    if ( is_plugin_active( 'lordicon-interactive-icons/lord-icon-wp.php' ) ) {
        $dir = plugin_dir_path(  __DIR__ ) . 'inc/lord-icon/';
        $dircontents = scandir($dir);
        foreach ($dircontents as $file) {
            $extension = pathinfo($file, PATHINFO_EXTENSION);
            if ($extension == 'json') {
                $icons[pathinfo($file, PATHINFO_FILENAME)] = pathinfo($file, PATHINFO_FILENAME);
            }
        }
    }
    return $icons;
...
function saasland_lordicon_exists($icon) {
    if ( !is_plugin_active( 'lordicon-interactive-icons/lord-icon-wp.php' ) ) {
        return false;
    }
...
function saasland_render_lordicon( $attributes, $content = null){
    ...
    if (!saasland_lordicon_exists($icon)) {
        return 'Invalid icon.';
    }
    ...
    $src = plugins_url( 'inc/lord-icon/'.$icon.'.json', dirname( __FILE__ ) );
